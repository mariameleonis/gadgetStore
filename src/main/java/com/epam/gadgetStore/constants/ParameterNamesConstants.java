package com.epam.gadgetStore.constants;

public final class ParameterNamesConstants {

	public static final String COMMAND_PARAMETER = "command";
	public static final String ID_PARAMETER = "id";
	public static final String PASSWORD_PARAMETER = "password";
	public static final String PHONE_NUMBER_PARAMETER = "phonenumber";
	public static final String EMAIL_PARAMETER = "email";
	public static final String FIRSTNAME_PARAMETER = "firstname";
	public static final String LASTNAME_PARAMETER = "lastname";
	public static final String IS_ADMIN_PARAMETER = "isAdmin";
	public static final String CARD_NUMBER_PARAMETER = "card-number";
	public static final String SECURITY_CODE_PARAMETER = "security-code";
	public static final String CARD_HOLDER_PARAMETER = "card-holder";
	public static final String EXPIRATION_DATE_PARAMETER = "date";
	public static final String ADDRESS_PARAMETER = "address";
	public static final String ORDER_STATUS_PARAMETER = "order-status";
	public static final String LANGUAGE_TO_CHANGE_PARAMETER = "languageToChange";
	public static final String PARENT_CATEGORY_PARAMETER = "parent-category";
	public static final String CATEGORY_NAME_PARAMETER = "category-name-";
	public static final String CATEGORY_ITEM_TITLE_PARAMETER = "category-item-title-";	
	public static final String NAME_PARAMETER = "name";
	public static final String DESCRIPTION_PARAMETER = "description";
	public static final String PRICE_PARAMETER = "price";
	public static final String PRODUCT_IMAGE_PARAMETER = "productImage";
	public static final String CATEGORY_PARAMETER = "category";
	public static final String BRAND_PARAMETER = "brand";
	
	private ParameterNamesConstants() {

	}
}
