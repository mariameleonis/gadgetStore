...
public class UpdateAdminProfileCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		User admin = (User) request.getSession().getAttribute(LOGGED_USER_ATTRIBUTE);

		fillUserFromRequest(admin, request);

		if (!isValidUserForm(request, response, admin)) {
			LOGGER.debug("Values in one or more fields are invalid.");
			forwardToPage(ADMIN_PROFILE_EDIT_FORM_JSP, request, response);
			return;
		}

		setEncryptedPassword(request, admin);

		try {
			userDAO.update(admin);
		} catch (DAOException e) {
			LOGGER.debug("Failed to update profile for user " + admin.getEmail() +"/n", e);
			throw new UnsupportedOperationException(e);
		}

		LOGGER.debug("Profile updated for user " + admin.getEmail());
		forwardToPage(ADMIN_PROFILE_JSP, USER_PROFILE_UPDATED_MESSAGE, request, response);
	}
}
